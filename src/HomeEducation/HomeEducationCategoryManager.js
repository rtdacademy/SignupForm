...
// Sheet components removed - now handled by parent component
...
// Additional icons that weren't in the first import
import {
  RotateCcw,
  Square,
  Triangle,
...
// Cloud functions no longer needed - categories are orphaned on families when deleted
...
const iconOptions = [
  ...
  { value: 'home', label: 'Home Education', icon: HomeIcon },
  ...
  { value: 'presentation', label: 'Meeting', icon: Presentation },
  ...
  // Family Management icons
  { value: 'award', label: 'Award', icon: Award },
  { value: 'flag', label: 'Flagged', icon: Flag },
  { value: 'star', label: 'Star Family', icon: Star },
  ...
  { value: 'sparkles', label: 'Exceptional', icon: Sparkles },
  ...
  { value: 'building2', label: 'Organization', icon: Building2 },
  { value: 'credit-card', label: 'Payment', icon: CreditCard },
  { value: 'user-check', label: 'Verified', icon: UserCheck },
];
...
const colorOptions = [
  ...
  { value: '#e74c3c', label: 'Danger' },
];

const HomeEducationCategoryManager = ({ onCategoryChange }) => {
  // Existing states
  const [categories, setCategories] = useState([]);
  const [categoryTypes, setCategoryTypes] = useState([]);
  const [newCategory, setNewCategory] = useState({ name: '', color: '', icon: '', type: '' });
  // Sheet state removed - now handled by parent component
  ...
  // Additional states for all categories and facilitator info
  const [allCategories, setAllCategories] = useState([]);
  const [facilitatorNames, setFacilitatorNames] = useState({});

  // New state for super admin facilitator selection
  const [selectedFacilitatorKey, setSelectedFacilitatorKey] = useState(null);
  ...
  const getEffectiveEmailKey = useCallback(() => {
    if (hasSuperAdminAccess() && selectedFacilitatorKey) {
      return selectedFacilitatorKey;
    }
    return current_user_email_key;
  }, [hasSuperAdminAccess, selectedFacilitatorKey, current_user_email_key]);

  // Fetch facilitator list for super admins and set default selected facilitator if not already set
  useEffect(() => {
    if (hasSuperAdminAccess()) {
      ...
      const unsubscribe = onValue(staffRef, (snapshot) => {
        if (snapshot.exists()) {
          ...
          setFacilitatorList(staffArray);
          if (!selectedFacilitatorKey) {
            setSelectedFacilitatorKey(user_email_key);
          }
        }
      });
  ...
  }, [hasSuperAdminAccess, user_email_key, selectedFacilitatorKey]);

  // Load categories for the effective facilitator key using memoized getEffectiveEmailKey
  useEffect(() => {
    const effectiveEmailKey = getEffectiveEmailKey();
    if (!effectiveEmailKey) return;

    const db = getDatabase();
    const categoriesRef = ref(db, `homeEducationCategories/${effectiveEmailKey}`);
    ...
    const unsubscribe = onValue(categoriesRef, (snapshot) => {
      if (snapshot.exists()) {
        ...
        // Only call onCategoryChange if it's provided
  ...
  // Load all categories and facilitator names
  useEffect(() => {
    const db = getDatabase();
    const allCategoriesRef = ref(db, 'homeEducationCategories');
    const staffRef = ref(db, 'staff');
    ...
    const unsubscribeAllCategories = onValue(allCategoriesRef, (snapshot) => {
      if (snapshot.exists()) {
        const categoriesData = snapshot.val();
        const allCategoriesArray = Object.entries(categoriesData).flatMap(([facilitatorKey, facilitatorCategories]) =>
          Object.entries(facilitatorCategories).map(([id, data]) => ({
            id,
            facilitatorKey,
            ...data,
          }))
  ...
  const handleAddCategoryType = async () => {
    ...
    const typeRef = ref(db, `homeEducationCategoryTypes/${typeId}`);
  ...
  const handleAddCategory = async () => {
    ...
    try {
      const effectiveEmailKey = getEffectiveEmailKey();
      console.log('Using email key:', effectiveEmailKey);
      
      const db = getDatabase();
      const categoryId = Date.now().toString();
      const newCategoryRef = ref(db, `homeEducationCategories/${effectiveEmailKey}/${categoryId}`);
      
      console.log('Saving category to Firebase...');
      ...
      // Switch to active tab to show the new category
      setActiveTab('active');
    } catch (error) {
  ...
  const handleCategoryAction = async (categoryId, action, facilitatorKey = getEffectiveEmailKey()) => {
    // Verify permissions
    if (!isStaffUser || (!hasSuperAdminAccess() && facilitatorKey !== user_email_key)) {
    ...
    if (facilitatorKey) {
      const db = getDatabase();
      const categoryRef = ref(db, `homeEducationCategories/${facilitatorKey}/${categoryId}`);
      ...
      if (action === 'delete') {
        ...
        try {
          ...
          showNotification(
            `Category deleted successfully. Note: Families may still have this category assigned, but it will no longer be visible.`
          );
  ...
  const handleUpdateCategoryType = async (categoryId, typeId, facilitatorKey = getEffectiveEmailKey()) => {
    const db = getDatabase();
    const categoryRef = ref(db, `homeEducationCategories/${facilitatorKey}/${categoryId}`);
            ...
            <div className="flex items-center space-x-2">
              <TooltipProvider>
                ...
                <Tooltip>
                      ...
                      <AlertDialog>
                        ...
                        <AlertDialogContent>
                          <AlertDialogHeader>
                            ...
                            <AlertDialogDescription>
                              This will permanently delete the category and remove it from all families.
                            </AlertDialogDescription>
                          </AlertDialogHeader>
                          <AlertDialogFooter>
                  ...
                  <TooltipContent>
                    <p>Delete: Permanently remove this category and remove it from ALL families</p>
                  </TooltipContent>
                </Tooltip>
              </TooltipProvider>
            </div>
  ...
  return (
    <>
      <div className="space-y-6">
          ...
          {/* Facilitator selector for super admins */}
          {hasSuperAdminAccess() && (
            <div className="mb-6 mt-2">
              ...
              <Select
                value={selectedFacilitatorKey}
                onValueChange={setSelectedFacilitatorKey}
              >
                ...
                <SelectContent>
                  {facilitatorList.map((facilitator) => (
                    <SelectItem key={facilitator.email_key} value={facilitator.email_key}>
                      <div className="flex items-center justify-between w-full">
                        <span>{facilitator.firstName} {facilitator.lastName}</span>
...
export default HomeEducationCategoryManager;