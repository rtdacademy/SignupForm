...
/**
 * AI-Enhanced Container Component
 * 
 * A non-collapsible container with integrated "Ask AI" functionality that extracts
 * content from the container body and sends it to the AI chat with proper context.
 * Matches the styling theme of AIAccordion but is always open.
 */
...
const AIContainer = ({ 
  ...
  const handleAskAI = () => {
    if (!onAskAI) {
      console.warn('onAskAI callback not provided to AIContainer');
      return;
    }

    try {
      // Extract content from the container body
        ...
        if (childrenElement && childrenElement.children.length > 0) {
          ...
          if (cleanedHTML.trim()) {
            originalJSX = React.createElement('div', {
              className: 'container-content-display static-content',
      ...
      console.error('Error extracting content from container:', error);
  ...
  return (
    <div className={`${className} transition-all duration-200 overflow-hidden`} {...props}>
      {/* Header */}
      <div className="bg-gradient-to-r from-purple-600 to-indigo-600 rounded-t-lg border border-purple-600 shadow-md">
        <div className="flex items-center justify-between px-6 py-4">
          ...
          {showHeaderButton && (
            <button
              onClick={handleAskAI}
              className="ml-3 inline-flex items-center gap-1.5 px-3 py-2 text-sm font-medium text-purple-700 bg-white hover:bg-purple-50 rounded-lg shadow-md hover:shadow-lg transition-all duration-200 transform hover:scale-105"
