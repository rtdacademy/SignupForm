...
/**
 * Database Function: syncStudentASNV2
 * Maintains ASN records in Realtime Database
 * V2 version with improved configuration
 */
const syncStudentASNV2 = onValueWritten({
  ref: '/students/{userEmailKey}/profile/asn',
  region: 'us-central1',
  memory: '256MiB',
  maxInstances: 50
}, async (event) => {
  const { userEmailKey } = event.params;
  const newASN = event.data.after?.val();
  const previousASN = event.data.before?.val();
