...
const createLogEntry = (stage, data = null, error = null) => {
  ...
  const logEntry = {
    timestamp,
    stage,
    functionName: 'updateIMathASGradeV2'
  };
...
/**
 * Updates IMathAS grades in the Firebase database
 * V2 version with improved configuration and error handling
 */
const updateIMathASGradeV2 = onRequest({
  secrets: ["API_KEY"],
  memory: '256MiB',
  timeoutSeconds: 60,
  maxInstances: 20,
  ...
  try {
    ...
    // Access the API key from Secret Manager
...
/**
 * Imports IMathAS grades from Cloud Storage
 * V2 version with improved configuration and error handling
 */
const importIMathASGradesV2 = onRequest({
  memory: '1GiB',
  timeoutSeconds: 540,
  maxInstances: 5,
  cors: true
...
// Export the V2 functions
module.exports = {
  updateIMathASGradeV2,
  importIMathASGradesV2
};